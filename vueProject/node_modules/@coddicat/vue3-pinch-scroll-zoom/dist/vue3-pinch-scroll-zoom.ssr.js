...
function _nonIterableRest() {
...
}var PinchScrollZoomAxis = /*#__PURE__*/function () {
  function PinchScrollZoomAxis(size, origin, translate, contentSize) {
    _classCallCheck(this, PinchScrollZoomAxis);

    _defineProperty(this, "_point", 0);

    _defineProperty(this, "_start1", 0);

    _defineProperty(this, "_start2", 0);
  ...
  _createClass(PinchScrollZoomAxis, [{
    key: "point",
    get: function get() {
      return this._point;
    }
    ...
    key: "touch",
    value: function touch(point) {
      this._start1 = point - this._point;
    }
  }, {
    key: "pinch",
    value: function pinch(point1, point2, scale) {
    ...
    key: "dragPinch",
    value: function dragPinch(point1, point2) {
    ...
    key: "dragTouch",
    value: function dragTouch(point) {
      this._point = point - this._start1;
    }
  }, {
    key: "checkAndResetToWithin",
    value: function checkAndResetToWithin(scale) {
      var _this$_contentSize;

      var contentSize = (_this$_contentSize = this._contentSize) !== null && _this$_contentSize !== void 0 ? _this$_contentSize : this._size;
...
(function() {
  ...
  setup: function setup(props, ctx) {
    ...
    var touch1 = false;
    var touch2 = false;
    var currentScale = vue.ref(props.scale);
    var startScale = vue.ref(props.scale);
    var axisX = new PinchScrollZoomAxis(props.width, props.originX, props.translateX, props.contentWidth);
    var axisY = new PinchScrollZoomAxis(props.height, props.originY, props.translateY, props.contentHeight);
    var zoomIn = vue.ref(false);
    var zoomOut = vue.ref(false);
    var stopDragListener = false;
    var startDragListener = false;
    var draggingListener = false;
    var scalingListener = false;
    ...
    var xPoint = vue.ref(axisX.point);
    var yPoint = vue.ref(axisY.point);
    var xOrigin = vue.ref(axisX.origin);
    var yOrigin = vue.ref(axisY.origin);
    ...
    var containerStyle = vue.computed(function () {
      var x = "".concat(xPoint.value, "px");
      var y = "".concat(yPoint.value, "px");
      var translate = "translate(".concat(x, ", ").concat(y, ") scale(").concat(currentScale.value, ")");
      var transformOrigin = "".concat(xOrigin.value, "px ").concat(yOrigin.value, "px");
      return {
        transform: translate,
    ...
    var getEmitData = function getEmitData() {
      return {
        x: axisX.point,
        y: axisY.point,
    ...
    var getBoundingTouchClientX = function getBoundingTouchClientX(touch) {
      if (el.value) {
    ...
    var getBoundingTouchClientY = function getBoundingTouchClientY(touch) {
      if (el.value) {
    ...
    var startDrag = function startDrag(touchEvent) {
      if (!props.draggable) {
        return;
      }
      ...
      var clientX1 = getBoundingTouchClientX(touchEvent.touches[0]);
      var clientY1 = getBoundingTouchClientY(touchEvent.touches[0]);

      if (touchEvent.touches.length > 1) {
        touch1 = true;
        touch2 = true;
        startScale.value = currentScale.value;
        var clientX2 = getBoundingTouchClientX(touchEvent.touches[1]);
        var clientY2 = getBoundingTouchClientY(touchEvent.touches[1]);
    ...
    var doDrag = function doDrag(touchEvent) {
      if (!props.draggable) return;
      throttleDoDrag(touchEvent);
    };

    var doStopScalingEvent = function doStopScalingEvent() {
      zoomIn.value = false;
      zoomOut.value = false;
    };

    var doDragEvent = function doDragEvent(touchEvent) {
    ...
    var throttleDoDrag = lodash.throttle(doDragEvent, props.throttleDelay);
    var stopScaling = lodash.debounce(doStopScalingEvent, 200);

    var submitDrag = function submitDrag() {
      updateAxis();
    ...
    var doScale = function doScale(touchEvent) {
      if (touchEvent.touches.length < 2 || !touch1 || !touch2) {
        checkWithin();
        return;
      }

      var t1 = touchEvent.touches[0];
      var t2 = touchEvent.touches[1];
      var distance = getDistance(getBoundingTouchClientX(t1), getBoundingTouchClientY(t1), getBoundingTouchClientX(t2), getBoundingTouchClientY(t2));
      var startDistance = getDistance(axisX.start1, axisY.start1, axisX.start2, axisY.start2);
      var scale = startScale.value * (distance / startDistance);
      submitScale(scale);
    };

    var submitScale = function submitScale(scale) {
    ...
    var doWheelScale = function doWheelScale(event) {
      event.preventDefault();
      var clientX = getBoundingTouchClientX(event);
      var clientY = getBoundingTouchClientY(event);
      axisX.pinch(clientX, clientY, currentScale.value);
      axisY.pinch(clientX, clientY, currentScale.value);
      var factor = 1 - event.deltaY * props.wheelVelocity;
      var scale = currentScale.value * factor;
      submitScale(scale);
    };

    var checkWithin = function checkWithin() {
      if (!props.within) {
        return;
      }
    ...
    vue.watch(function () {
      return props.originX;
    }, function (val) {
      axisX.setOrigin(val);
    });
    vue.watch(function () {
      return props.originY;
    }, function (val) {
      axisY.setOrigin(val);
    });
    vue.watch(function () {
  ...
  return vue.openBlock(), vue.createElementBlock("div", {
    ref: "el",
    class: vue.normalizeClass(["pinch-scroll-zoom", _ctx.componentClass]),
    ...
    onMousemove: _cache[1] || (_cache[1] = function () {
      return _ctx.doDrag && _ctx.doDrag.apply(_ctx, arguments);
    })
  }, [vue.createElementVNode("div", {
    ref: "content",
    class: "pinch-scroll-zoom__content",
    style: vue.normalizeStyle(_ctx.containerStyle)
  }, [vue.renderSlot(_ctx.$slots, "default")], 4)], 38);
